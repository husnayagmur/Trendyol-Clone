  ...
  <div className="bg-dropdownBg z-10 w-[15%] h-[calc(79vh)] rounded-b-md ">
    <ul className="flex flex-col">
      {allCategories.map((category, index) => (
        <li
          key={index}
          className={`group flex items-center space-x-3 px-4 py-4 cursor-pointer whitespace-nowrap font-source-sans-pro justify-between ${
        ...
        >
          <div className="flex items-center space-x-3">
            ...
            />
            <span
              className={`text-mediumDark-gray text-sm font-semibold ${
                hoveredCategory === category ? 'text-orange' : 'group-hover:text-orange'
              }`}
            >
              {category.name}
            </span>
