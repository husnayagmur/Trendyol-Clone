...
const Home = () => {
  ...
  const menuItems = [
    { icon: <RxHamburgerMenu size={23} />, text: "TÜM KATEGORİLER", href: "#", className: "text-xs pr-12 pl-10" },
    { text: "Kadın", href: "#" },
    { text: "Erkek", href: "#" },
    { text: "Anne & Çocuk", href: "#" },
    { text: "Ev & Yaşam", href: "#" },
    { text: "Süpermarket", href: "#" },
    { text: "Kozmetik", href: "#" },
    { text: "Ayakkabı & Çanta", href: "#" },
    { text: "Elektronik", href: "#" },
    { iconSrc: "/icons/cokSatanlar.svg", text: "Çok Satanlar", href: "#" },
    { iconSrc: "/icons/flash.svg", text: "Flaş Ürünler", href: "#", isGradient: true },
  ];
  return (
    ...
    {menuItems.map((item, index) => (
      <li
        key={index}
        className={`flex items-center space-x-1 ${item.className || ""} ${item.isGradient ? "group" : ""}`}
      >
        {item.iconSrc && (
          <img
            src={item.iconSrc}
        ...
        <Link
          href={item.href}
          className={`cursor-pointer font-bold font-source-sans-pro ${
            item.isGradient
              ? "group-hover:bg-gradient-to-r group-hover:from-orange group-hover:to-pink-600 group-hover:bg-clip-text group-hover:text-transparent"
              : "hover:text-orange"
          } duration-150`}
        >
          {item.text}
